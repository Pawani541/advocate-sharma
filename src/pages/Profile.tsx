...
const Profile = () => {
  ...
  const [photo, setPhoto] = useState<string>(
    localStorage.getItem('advocatePhoto') || ''
  );
      ...
      <div style={{ width: '100%', maxWidth: '500px' }}>
        ...
        <div style={{ background: 'var(--dark-card)', border: '1px solid var(--dark-border)', borderRadius: '16px', padding: '48px', textAlign: 'center' }}>
          ...
          <div style={{ position: 'relative', width: '110px', height: '110px', margin: '0 auto 12px' }}>
            <div style={{ width: '110px', height: '110px', borderRadius: '50%', background: 'var(--gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', border: '3px solid var(--gold)' }}>
              {photo ? (
            ...
            {user?.role === 'admin' && (
              <>
                <label htmlFor="photo-upload" style={{ position: 'absolute', bottom: '2px', right: '2px', background: 'var(--gold)', borderRadius: '50%', width: '34px', height: '34px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: '2px solid #0a0a0f', zIndex: 10 }}>
          ...
          {user?.role === 'admin' && (
            <p style={{ color: 'var(--gold)', fontSize: '12px', marginBottom: '20px', letterSpacing: '0.05em' }}>
              Click the camera icon to upload advocate photo
            </p>
          )}

          <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '4px' }}>{user?.name}</h1>
          <div style={{ color: 'var(--gold)', fontSize: '13px', marginBottom: '32px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{user?.role}</div>

          {[
            { label: 'Email', value: user?.email },
            { label: 'Phone', value: (user as any)?.phone || 'Not provided' },
            { label: 'Occupation', value: (user as any)?.occupation || 'Not provided' }
          ].map(f => (
