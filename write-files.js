...
const AdminDashboard = () => {
  ...
  const [photo, setPhoto] = useState(localStorage.getItem('advocatePhoto') || '');
  ...
  const handlePhoto = (e) => {
    ...
    reader.onload = () => {
      localStorage.setItem('advocatePhoto', reader.result);
  ...
  const statusColor = (s) => s === 'approved' ? '#22c55e' : s === 'rejected' ? '#ef4444' : 'var(--gold)';
              ...
              cases.map((c, i) => (
                <div key={i} style={{ background: 'var(--dark-card)', border: '1px solid var(--dark-border)', borderRadius: '12px', padding: '24px', marginBottom: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                    ...
                    <div style={{ color: statusColor(c.status), padding: '5px 14px', borderRadius: '100px', fontSize: '12px', textTransform: 'capitalize', fontWeight: 600, border: '1px solid currentColor' }}>{c.status}</div>
                  </div>
              ...
              meetings.map((m, i) => (
                <div key={i} style={{ background: 'var(--dark-card)', border: '1px solid var(--dark-border)', borderRadius: '12px', padding: '24px', marginBottom: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                    ...
                    <div style={{ color: statusColor(m.status), padding: '5px 14px', borderRadius: '100px', fontSize: '12px', textTransform: 'capitalize', fontWeight: 600, border: '1px solid currentColor' }}>{m.status}</div>
                  </div>
